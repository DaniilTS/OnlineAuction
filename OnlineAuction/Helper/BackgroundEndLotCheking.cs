    ...
    public class BackgroundEndLotCheking
    {
        ...
        public async Task ChekLot()
        {
            ...
            foreach (var lot in lots)
            {
                ...
                {
                    if (lot.User.UserName == lot.WiningUserName)
                    {
                        var message = $"{lot.User.UserName}, никто не купил ваш лот: {lot.Name}";

                        await emailService.SendEmailAsync(lot.User.Email, "OnlineAuction", message);
                    }
                    else
                    {
